...
from .serializers import (RegisterationApiSerializer,CustomAuthTokenSerializer,
...
class ResendActivationApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not user.is_verficated:
                token = get_tokens_for_user(user)
                message = EmailMessage('email/email_verify.tpl',{'token':token},'admin@gmail.com',to=[user.email])
                message.send()
                return Response(data={'details':'Send Activation Link for Email; checkout'},status=status.HTTP_200_OK)
            return Response(data={'details':'This user has already been confirmed.'},status=status.HTTP_400_BAD_REQUEST)
        
        except CustomUser.DoesNotExist:
            return Response(data={"details":"User is Not Found"},status=status.HTTP_404_NOT_FOUND)
# ________________________________________________
...
class ChangePasswordApiView(generics.GenericAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if user.check_password(ser_data.validated_data['old_password']):
            ...
            user.save()
            return Response(data={'details':'The set new password is successfuly'},status=status.HTTP_200_OK)
        return Response(data={'details':'The Current password is Invalid'},status=status.HTTP_400_BAD_REQUEST)
...
class ResetPasswordApiView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        message = EmailMessage('email/reset_password.tpl',{'token':token},'admin@gmail.com',to=[email])
        message.send()
        
        return Response(data={'details':'Reset Password Link is send email; checkout'},status=status.HTTP_200_OK)
# ________________________________________________
    ...
    def post(self, request, token):
        try:
            ...
            if not user.check_password(password):
                ...
                user.save()
                return Response(data={'details':'The user password has been successfully changed.'},status=status.HTTP_200_OK)
            return Response(data={'details':'This password has been used before.'},status=status.HTTP_400_BAD_REQUEST)
        
        except ExpiredSignatureError:
            return Response({"details": "This link has expired to change the password"})

        except ExpiredSignatureError:
            return Response({"details": "This link is not valid for changing the password."})
# ________________________________________________
